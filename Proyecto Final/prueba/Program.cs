...
namespace Tarea4
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            testResults.Add(logininvalido(driver));
            testResults.Add(loginvalido(driver));
            testResults.Add(VerContraseña(driver));
            testResults.Add(LoginVAcio(driver));
            testResults.Add(BtnSalirMenu(driver));
        ...
        static string NavegacionLibros(IWebDriver driver)
        {
            ...
            inputUsuario.SendKeys("Jhon");

            IWebElement inputContrasena = driver.FindElement(By.Name("password"));
            inputContrasena.SendKeys("12345");

            IWebElement btnIniciarSesion = driver.FindElement(By.XPath("//button[contains(text(),'Iniciar Sesión')]"));
        ...
        static string infoAutores(IWebDriver driver)
        {
            ...
            if (modals.Count > 0)
            {
                ...
                IWebElement btnCerrarModal = driver.FindElement(By.XPath("//button[@class='close']"));
        ...
        static string logininvalido(IWebDriver driver)
        {
            linkPagina(driver);

            IWebElement inputUsuario = driver.FindElement(By.Name("username"));
            inputUsuario.SendKeys("juanito12345");

            IWebElement inputContrasena = driver.FindElement(By.Name("password"));
            inputContrasena.SendKeys("");
            ...
            try
            {
                capturasPantallas(driver, "logininvalido");
                IAlert alert = driver.SwitchTo().Alert();
                alert.Dismiss();
                Console.WriteLine("Se ha cerrado el cuadro de diálogo de alerta.");
            }
            catch (NoAlertPresentException)
            {
                Console.WriteLine("No se encontró ningún cuadro de diálogo de alerta.");
            }

            Thread.Sleep(2000);

            string urlActual = driver.Url;
            if (urlActual.Contains("index.php"))
            {
                Console.WriteLine("Éxito: No se pudo iniciar sesión con credenciales inválidas.");
                return "Exitoso";
            }
            else
            {
                Console.WriteLine("Error: Se inició sesión con credenciales inválidas.");
                return "Fallido";
            }

        }
        static string loginvalido(IWebDriver driver)
        {
            linkPagina(driver);
            ...
            if (!urlActual.Contains("index.php"))
            {
                Console.WriteLine("Éxito: Se inició sesión con credenciales válidas.");
                capturasPantallas(driver, "loginValido");
                Thread.Sleep(3000);
            ...
            {
                Console.WriteLine("Error: No se pudo iniciar sesión con credenciales válidas.");
                return "Fallido";
            }
        }
        static string VerContraseña(IWebDriver driver)
        {
            linkPagina(driver);
            ...
            // Encontrar el botón de ver contraseña
            IWebElement btnVerContrasena = driver.FindElement(By.Id("passwordVisibility"));
            btnVerContrasena.Click();
            capturasPantallas(driver, "VerContrasena");
            Thread.Sleep(5000);

            return "Exitoso";
        }

        // Caso de prueba 12: Verificar si el input de username tiene límites de caracteres
        static string LoginVAcio(IWebDriver driver)
        {
            linkPagina(driver);

            // Ingresar un username vacío
            IWebElement inputUsuario = driver.FindElement(By.Name("username"));
            inputUsuario.Clear(); 
            ...
            try
            {
                capturasPantallas(driver, "LoginVacio");
                IAlert alert = driver.SwitchTo().Alert();
                Console.WriteLine("Mensaje de alerta presente: " + alert.Text);
                alert.Dismiss();
                return "Exitoso";
            }
            catch (NoAlertPresentException)
            {
                Console.WriteLine("No se encontró ningún mensaje de alerta.");
                return "Fallido";
            }
        }
        ...
        static string BtnSalirMenu(IWebDriver driver)
        {
            linkPagina(driver);
            ...
            IWebElement btnSalirMenu = driver.FindElement(By.LinkText("Salir"));
            btnSalirMenu.Click();
            Thread.Sleep(2000);
            ...
            if (urlActual.Contains("index.php"))
            {
                Console.WriteLine("Éxito: El botón de salir en el menú te lleva al login de nuevo.");
                capturasPantallas(driver, "BtnSalirMenu");
                return "Exitoso";
            }
            else
            {
                Console.WriteLine("Error: El botón de salir en el menú no te lleva al login de nuevo.");
                return "Fallido";
            }
        }
